...
export default function OrderOfPlay() {
  ...
  const deleteDay = async (dayIndex) => {
    ...
    const confirmDelete = window.confirm(
      "This day will be permanently deleted from the database. Are you sure?",
    );
    ...
    try {
      ...
      // Collect all matches from the deleted day's grid
      ...
      // Get remaining matches of the deleted day
      const deletedRemaining = dayToDelete.remaining || [];

      // Get the previous day
      const previousDayIndex = dayIndex - 1;
      const previousDay = days[previousDayIndex];

      // Get previous day's remaining matches
      const previousRemaining = previousDay.remaining || [];

      // Restore deleted matches to previous day's remaining
      ...
      // Remove the deleted day and update previous day
      const updatedDays = days
      ...
      console.log("RESTORED REMAINING:", uniqueMatches);
      console.log("UPDATED DAYS:", updatedDays);

      // Delete the day from database
      await axios.delete(
        `${import.meta.env.VITE_APP_BACKEND_URL}/api/order-of-play/${selectedEventId}/${dayToDelete.date}`,
        {
          withCredentials: true,
        },
      );
      ...
      //setNotPlacedMatches(updatedDays[0]?.remaining || []);

      // Update session storage
      sessionStorage.setItem("orderPlayDays", JSON.stringify(updatedDays));

      toast.success(`Day ${dayIndex + 1} deleted successfully`);
    } catch (err) {
      console.error("DELETE DAY ERROR:", err);

      toast.error(err.response?.data?.message || "Error deleting day");
    }
  };
  /* ================= BUILD GRID ================= */
